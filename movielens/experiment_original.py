...
def experiment(
    ...
    # load dataset
    dataset_path = f"../smart-climate/data/smart-climate/smart-climate-train-trajectories-v9.pkl"
    ...
    act_dim = 1
    total_actions = np.unique(np.concatenate(actions)).shape[0]
    print('=' * 50)
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            # print(f"timesteps.shape: {len(timesteps)}")
            # print(f"timesteps: {timesteps[-1]}")
            timesteps[-1][timesteps[-1] >= max_ep_len] = max_ep_len-1  # padding cutoff
            # print(f"timesteps after padding cutoff: {timesteps[-1]}")
