...
def experiment(
    ...
    def eval_episodes(target_rew):
        def fn(model):
            ...
            eval_action_errors = []
            pred_actions_all_ep, target_actions_all_ep = [], []
            for _ in tqdm(range(num_eval_episodes), disable=False):
                with torch.no_grad():
                    if model_type == 'dt':
                        ret, length, acc, eval_action_error, pred_actions, target_actions = evaluate_episode_rtg(
                            env,
                            state_dim,
                            act_dim,
                            model,
                ...
                pred_actions_all_ep += pred_actions
                target_actions_all_ep += target_actions
                
                eval_action_errors.append(eval_action_error)
            
            total_achievable_reward = np.sum((np.array(target_actions_all_ep) >= 3.5).astype(int))
            total_achievable_reward_in_100 = np.sum((np.array(target_actions_all_ep) >= 3.5).astype(int)) * 100 / len(target_actions_all_ep)

            total_reward_achieved =  np.sum(returns)
            total_reward_achieved_out_of_100 = np.sum(returns) * 100 / len(target_actions_all_ep)

            proportion_of_the_achieved_to_achievable = (total_reward_achieved / total_achievable_reward) * 100
            ...
            return {
                ...
                f'target_{target_rew}_overall_accuracy_mean': np.mean(accuracies),
                f'target_{target_rew}_different_unique_target_actions': sorted(set(target_actions_all_ep)),
                f'target_{target_rew}_different_unique_pred_actions': sorted(set(pred_actions_all_ep)),
                f'target_{target_rew}_total_achievable_rewards': total_achievable_reward,
                f'target_{target_rew}_total_achievable_rewards_out_of_100': total_achievable_reward_in_100,
                f'target_{target_rew}_total_reward_achieved':total_reward_achieved,
                f'target_{target_rew}_total_reward_achieved_out_of_100': total_reward_achieved_out_of_100,
                f'proportion_of_the_achievable_reward_achieved': proportion_of_the_achieved_to_achievable,
            }
        return fn
