...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        ...
        if self.is_cross_attention:
            self.c_attn = Conv1D(2*n_state, nx).to('cuda')
            self.q_attn = Conv1D(n_state, nx).to('cuda')
        else:
        ...
        # print(f"IN THE ATTENTION MODULE: nx={nx}, n_ctx={n_ctx}")
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            # print(f"Device of hidden states while doing masked self-attention: {hidden_states.device}")
            # print(f"hidden_states.shape: {hidden_states.shape}")
        ...
        if layer_past is not None:
            # print(f"layer_past is not None")
        ...
        # print(f"a.shape before merging heads: {a.shape}")
        a = self.merge_heads(a)
        # print(f"a.shape after merging heads: {a.shape}")
        a = self.c_proj(a)
        a = self.resid_dropout(a)
        # print(f"a.shape after final steps: {a.shape}")
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        if config.fusion_strategy == 'cross':
            config.add_cross_attention = True
    ...
    def forward(
        ...
        # print(f"forward method of transformers")
        ...
        # print(f"head_mask shape: {len(head_mask)}")
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                # print(f"Block: {block}")
                # print(f"head_mask[{i}]: {head_mask[i]}")
                # print(f"hidden_states.shape: {hidden_states.shape}")
                outputs = block(
                    hidden_states,
                    layer_past=layer_past,
